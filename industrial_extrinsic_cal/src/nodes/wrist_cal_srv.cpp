...
public:
    ...
    : nh_(nh), P_(NULL), problem_initialized_(false), total_observations_(0), scene_(0)
  {
    ...
    {
      ros::topic::waitForMessage<sensor_msgs::CameraInfo>(camera_info_wait_topic.c_str());
    }
  ...
  void init_blocks()
  {
    ...
    {
      if (all_targets_[i]->pub_rviz_vis_)
  ...
  bool observationCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    for (int i = 0; i < all_cameras_.size(); i++)
    {
      ...
      industrial_extrinsic_cal::Cost_function cost_type =
          industrial_extrinsic_cal::cost_functions::CameraReprjErrorWithDistortionPK;
      int total_pts = 0;
      for (int j = 0; j < all_targets_.size(); j++)
      {  // add all targets to the camera
      ...
      {  // add a new cost to the problem for each observation
        ...
        for (int k = 0; k < num_observations; k++)
        {
          ...
          /*
          if(k<3){
      ROS_ERROR("target_pb = %ld extrinsics %ld intrinsics = %ld",(long int ) &(target_pb[0]),(long int) &extrinsics[0],
      (long int ) &intrinsics[0]); Pose6d
      P(extrinsics[3],extrinsics[4],extrinsics[5],extrinsics[0],extrinsics[1],extrinsics[2]); P.show("camera to world
      initial conditions"); Pose6d TP(target_pb[3],target_pb[4],target_pb[5],target_pb[0],target_pb[1],target_pb[2]);
      ...
      ROS_ERROR("Target Point %lf %lf %lf observed %lf %lf Residual = %lf %lf",point.x, point.y, point.z, image_x,
      image_y, resid[0],resid[1]);
          }
          */
  ...
  bool runCallBack(industrial_extrinsic_cal::cal_srv_solveRequest& req,
                   industrial_extrinsic_cal::cal_srv_solveResponse& res)
  {
    char msg[255];
    ...
    Pose6d P(all_cameras_[0]->camera_parameters_.pb_extrinsics[3], all_cameras_[0]->camera_parameters_.pb_extrinsics[4],
             all_cameras_[0]->camera_parameters_.pb_extrinsics[5], all_cameras_[0]->camera_parameters_.pb_extrinsics[0],
    ...
    if (summary.termination_type != ceres::NO_CONVERGENCE)
    {
      ...
      else
      {
        ...
        Pose6d Pf(
            all_cameras_[0]->camera_parameters_.pb_extrinsics[3], all_cameras_[0]->camera_parameters_.pb_extrinsics[4],
            all_cameras_[0]->camera_parameters_.pb_extrinsics[5], all_cameras_[0]->camera_parameters_.pb_extrinsics[0],
            all_cameras_[0]->camera_parameters_.pb_extrinsics[1], all_cameras_[0]->camera_parameters_.pb_extrinsics[2]);
  ...
  bool loadCallBack(industrial_extrinsic_cal::FileOp::Request& req, industrial_extrinsic_cal::FileOp::Response& resp)
  {
    ...
    if (problem_initialized_ != true)
    {  // scene_id=0, problem re-initialized
