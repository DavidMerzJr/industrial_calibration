...
{
calPanel::calPanel(QWidget* parent) : rviz::Panel(parent)
{
  ...
  calibration_selection_->addItem("WristCalSrv");   // Either Camera or Target on EOAT, uses TF to get IC for Camera to
                                                    // Target Transform
  calibration_selection_->addItem("ICalSrv");       // Either Camera or Target on EOAT, uses TF to get IC for Camera to
                                                    // Target Transform
  calibration_selection_->addItem("StereoCalSrv");  // Either Cameras or Target on EOAT, uses TF to get IC for Camera to
                                                    // Target Transform
  calibration_selection_->addItem("RailICalSrv");   // uses the 8D method where target to rail is changed after each
                                                    // sweep of rail, must happen at least once
